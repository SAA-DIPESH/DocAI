...
class ContextBuilder:
    ...
    def __init__(self):
        ...
        self.evidence_collection = self.db[evidence_collection_name]
        self.win_theme_collection = self.db[win_theme_collection_name]
        self.evaluation_criteria_collection = self.db[evaluation_criteria_collection_name]
    ...
    @staticmethod
    def _get_status(is_regenerate: bool) -> str:
        return "Regenerating" if is_regenerate else "Active"

    # ------------------------------------------------------------------
    # Tender section plan
    # NOTE: your plan document's top-level status field is `PlanStatus`,
    # not `Status` (only the *evidence* doc has a top-level `Status`).
    # If you actually want to filter plans by regenerate/active state,
    # confirm what values PlanStatus takes for that ("ValidationRequired"
    # in your sample isn't "Regenerating"/"Active"). Until confirmed,
    # this matches on TenderId + CompanyId only and takes the most
    # recently modified plan.
    # ------------------------------------------------------------------
    def _find_plan_document(self, tender_id: str, company_id: str) -> Dict[str, Any]:
        document = self.section_collection.find_one(
            {"TenderId": tender_id, "CompanyId": company_id},
            sort=[("ModifiedDate", -1), ("CreatedDate", -1)],
        )
        return document or {}
    ...
    def get_tender_sections(
        ...
        document = self._find_plan_document(tender_id, company_id)
        if not document:
            return []

        proposal_groups = document.get("JsonOutput", {}).get("ProposalGroups", [])
        sections = []
        for group in proposal_groups:
            group_name = group.get("GroupName")
            for section in group.get("Sections", []):
    ...
    def get_proposal_metadata(
        ...
        document = self._find_plan_document(tender_id, company_id)
        if not document:
            return {"ProposalPlanId": "", "ProposalId": ""}

        proposal_plan_id = _first_non_empty(
            document, "ProposalPlanId", "proposalPlanId", "proposal_plan_id"
        )
        proposal_id = _first_non_empty(document, "ProposalId", "proposalId")

        # Never fall back to raw _id here — if you must, cast to str explicitly.
        if not proposal_plan_id and document.get("_id") is not None:
            proposal_plan_id = str(document["_id"])
    ...
    # ------------------------------------------------------------------
    # Evidence summaries — matches your new schema:
    # JsonOutput.EvidenceSummaries[].CanonicalRequirementId
    # ------------------------------------------------------------------
    def add_evidence_summary(
        self,
        sections: List[Dict[str, Any]],
        ...
        for section in sections:
            ...
            pipeline = [
                ...
                {"$unwind": "$JsonOutput.EvidenceSummaries"},
                {
                    "$match": {
                        "JsonOutput.EvidenceSummaries.CanonicalRequirementId": {
                            "$in": requirement_ids
                ...
                {"$replaceRoot": {"newRoot": "$JsonOutput.EvidenceSummaries"}},
            ]
    ...
    def get_win_themes(self, company_id: str, status: str) -> List[Dict[str, Any]]:
        pipeline = [
            {
                "$match": {
                    "$or": [
                        {"CompanyId": company_id, "Status": status},
                        {"company_id": company_id, "status": status},
                        {"generated_themes.company_id": company_id, "Status": status},
                    ]
                }
            },
            {"$unwind": {"path": "$generated_themes", "preserveNullAndEmptyArrays": True}},
            {"$project": {"_id": 0, "theme": {"$ifNull": ["$generated_themes", "$$ROOT"]}}},
            {"$replaceRoot": {"newRoot": "$theme"}},
        ]
        themes = list(self.win_theme_collection.aggregate(pipeline))
        if themes:
            return themes
        ...
        return list(self.win_theme_collection.aggregate(fallback_pipeline))

    def get_evaluation_criteria(self, tender_id: str, company_id: str) -> List[Dict[str, Any]]:
        document = self.evaluation_criteria_collection.find_one(
            {
                "$or": [
                    {"CompanyId": company_id, "TenderId": tender_id},
        ...
        criteria = (
            document.get("EvaluationCriteria")
            or document.get("Criteria")
            or document.get("JsonOutput", {}).get("EvaluationCriteria")
            or document.get("JsonOutput", {}).get("Criteria")
            or []
        )
        return criteria if isinstance(criteria, list) else [criteria]

    def add_evaluation_criteria(
        self, sections: List[Dict[str, Any]], tender_id: str, company_id: str
    ) -> List[Dict[str, Any]]:
        criteria = self.get_evaluation_criteria(tender_id=tender_id, company_id=company_id)
        for section in sections:
            section.setdefault("EvaluationCriteria", criteria)
        return sections

    def build_context(
        self, tender_id: str, company_id: str, is_regenerate: bool
    ) -> Dict[str, Any]:
        status = self._get_status(is_regenerate)

        sections = self.get_tender_sections(tender_id=tender_id, company_id=company_id)
        proposal_metadata = self.get_proposal_metadata(tender_id=tender_id, company_id=company_id)

        sections = self.add_evidence_summary(
            sections=sections, tender_id=tender_id, company_id=company_id, status=status
        )
        sections = self.add_evaluation_criteria(
            sections=sections, tender_id=tender_id, company_id=company_id
        )
        ...
        result = {
            ...
            "SourceStatus": status,
            "Sections": sections,
            "WinThemes": self.get_win_themes(company_id=company_id, status=status),
        }

        # Final safety net: strip out any stray ObjectId/datetime before returning.
